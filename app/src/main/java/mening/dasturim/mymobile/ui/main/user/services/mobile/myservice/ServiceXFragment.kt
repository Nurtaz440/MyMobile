package mening.dasturim.mymobile.ui.main.user.services.mobile.myservice
...
class ServiceXFragment : BaseFragment<FragmentInternetBinding, ServiceXFragmentVM>() {
    ...
    fun getUzmobile(){
        val query: Query =
            firebaseFirestore.collection("UzMobile").document("Xizmatlar").collection("tezkor otkazmalar")
  ...
  fun getUcell(){
        val query: Query =
            firebaseFirestore.collection("Ucell").document("Xizmatlar").collection("Mobile ASBT")
    ...
    override val vm: ServiceXFragmentVM
        get() = ViewModelProvider(this).get(ServiceXFragmentVM::class.java)
}