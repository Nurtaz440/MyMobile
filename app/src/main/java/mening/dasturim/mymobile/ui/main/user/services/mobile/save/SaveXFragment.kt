package mening.dasturim.mymobile.ui.main.user.services.mobile.save
...
class SaveXFragment : BaseFragment<FragmentInternetBinding, SaveXFragmentVM>() {
    ...
    fun getUzmobile(){
        val query: Query =
            firebaseFirestore.collection("UzMobile").document("Xizmatlar").collection("esim")
 ...
 fun getUcell(){
        val query: Query =
            firebaseFirestore.collection("Ucell").document("Xizmatlar").collection("Raqamni saqlab qolish")
    ...
    override val vm: SaveXFragmentVM
        get() = ViewModelProvider(this).get(SaveXFragmentVM::class.java)
}