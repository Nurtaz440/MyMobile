package mening.dasturim.mymobile.ui.main.user.tariff.welcom
...
class WelcomFragment: BaseFragment<FragmentInternetBinding, WelcomFragmentVM>() {
    private lateinit var rateAdapter: WelcomFragmentAdapter
    ...
    fun setUp() {
        ...
        list = ArrayList()
        rateAdapter = WelcomFragmentAdapter(requireContext()) {
    ...
    override val vm: WelcomFragmentVM
        get() = ViewModelProvider(this).get(WelcomFragmentVM::class.java)


    fun getUcell(){
        val query: Query =
            firebaseFirestore.collection("Ucell").document("Tariflar")
                .collection("Tarmoq ichida qulay")
    ...
    fun getBeeline(){
        val query: Query =
            firebaseFirestore.collection("Beeline").document("Tariflar").collection("Welcome")
