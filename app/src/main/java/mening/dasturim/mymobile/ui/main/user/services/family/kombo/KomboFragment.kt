package mening.dasturim.mymobile.ui.main.user.services.family.kombo
...
class KomboFragment: BaseFragment<FragmentInternetBinding, KomboFragmentVM>() {
   ...
   fun getMobiUz(){
        val query: Query =
            firebaseFirestore.collection("MobiUz").document("Xizmatlar").collection("SIM-kartani almashtirish")
  ...
  fun getUzMobile(){
        val query: Query =
            firebaseFirestore.collection("UzMobile").document("Xizmatlar").collection("YHQ jarimalari haqida xabar xizmati")
 ...
 fun getBeeline(){
        val query: Query =
            firebaseFirestore.collection("Beeline").document("Xizmatlar").collection("Internet-paket 300 MB")
    ...
    override val vm: KomboFragmentVM
        get() = ViewModelProvider(this).get(KomboFragmentVM::class.java)
}