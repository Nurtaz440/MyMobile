...
class MinutsPocket: BaseFragment<FragmentInternetPocketsBinding, MinutsPocketVM>() {
    private lateinit var fragmentAdapter: MinutsPocketVP
    ...
    fun setUp() {
        ...
        fragmentAdapter = MinutsPocketVP(requireFragmentManager(), requireActivity().lifecycle)
            ...
            if (it != null) {
                when (it) {
                    Companies.UZMOBILE -> {
                        ...
                        firebaseFirestore.collection("UzMobile").document("Daqiqalar").get()
                    ...
                    Companies.MOBIUZ -> {
                        ...
                        firebaseFirestore.collection("MobiUz").document("Daqiqalar").get()
                            .addOnCompleteListener(object : OnCompleteListener<DocumentSnapshot?> {
                                override fun onComplete(p0: Task<DocumentSnapshot?>) {
                                    if (p0.isSuccessful) {
                                        Log.d("Route Fragment", "onComplete: query " + p0)
                    ...
                    Companies.UCELL -> {
                        ...
                        firebaseFirestore.collection("Ucell").document("Daqiqalar").get()
                            .addOnCompleteListener(object : OnCompleteListener<DocumentSnapshot?> {
                                override fun onComplete(p0: Task<DocumentSnapshot?>) {
                                    if (p0.isSuccessful) {
                                        Log.d("Route Fragment", "onComplete: query " + p0)
                    ...
                    Companies.BEELINE -> {
                        ...
                        firebaseFirestore.collection("Beeline").document("Daqiqalar").get()
                            .addOnCompleteListener(object : OnCompleteListener<DocumentSnapshot?> {
                                override fun onComplete(p0: Task<DocumentSnapshot?>) {
                                    if (p0.isSuccessful) {
                                        Log.d("Route Fragment", "onComplete: query " + p0)
    ...
    override val vm: MinutsPocketVM
        get() = ViewModelProvider(this).get(MinutsPocketVM::class.java)
