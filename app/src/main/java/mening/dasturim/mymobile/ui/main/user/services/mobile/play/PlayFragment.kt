package mening.dasturim.mymobile.ui.main.user.services.mobile.play
...
class PlayFragment : BaseFragment<FragmentInternetBinding, PlayFragmentVM>() {
    ...
    fun getUzmobile(){
        val query: Query =
            firebaseFirestore.collection("UzMobile").document("Xizmatlar").collection("qabul qilnmagan qo'ng'iroq")
  ...
  fun getUcell(){
        val query: Query =
            firebaseFirestore.collection("Ucell").document("Xizmatlar").collection("play mobile")
    ...
    override val vm: PlayFragmentVM
        get() = ViewModelProvider(this).get(PlayFragmentVM::class.java)
}