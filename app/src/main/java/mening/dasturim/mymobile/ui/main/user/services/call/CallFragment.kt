...
import mening.dasturim.mymobile.ui.main.user.services.profite.ProfiteVM
import mening.dasturim.mymobile.utils.ViewUtils

class CallFragment  : BaseFragment<FragmentInternetPocketsBinding, CallFragmentVM>() {
    private lateinit var firebaseFirestore: FirebaseFirestore
    lateinit var profiteAdapter: CallFragmentVP
    override fun onBound() {
        setUp()
    }
    fun setUp(){
        ViewUtils.fadeIn(binding.progressBar)
        profiteAdapter= CallFragmentVP(requireFragmentManager(),requireActivity().lifecycle)
                                    ...
                                    if (p0.isSuccessful) {
                                        ...
                                        p0.result?.let {
                                            ...
                                            val list: List<String> =
                                                it.get("collections3") as ArrayList<String>
    ...
    override val vm: CallFragmentVM
        get() = ViewModelProvider(this).get(CallFragmentVM::class.java)
}